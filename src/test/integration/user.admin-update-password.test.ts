...
describe("Integration: PATCH /api/v1/users/admin/:userId/update-password", () => {
  it("updates user password successfully (ADMIN) and new password works for login", async () => {
    ...
    const targetUser = await User.findOne({
      "contact.email": TEST_USERS.NURSE.email,
    }).lean();
    ...
    const res = await client(adminToken)
      .patch(
        `/api/v1/users/admin/${targetUser!._id.toString()}/update-password`,
      )
    ...
    const loginWithNewPassword = await client()
    ...
    const loginWithOldPassword = await client()
      .post("/api/v1/auth/login")
      .send({
        identifier: TEST_USERS.NURSE.email,
  ...
  it("returns 404 when user is not found", async () => {
    ...
    const res = await client(adminToken)
      .patch("/api/v1/users/admin/507f1f77bcf86cd799439011/update-password")
      .send({
        password: "NewPassword123!",
      });
