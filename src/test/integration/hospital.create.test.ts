...
describe('Integration: POST /api/v1/hospitals', () => {
  it('creates a hospital successfully (ADMIN)', async () => {
    ...
    const res = await client(adminToken).post('/api/v1/hospitals').send(validHospitalPayload);
  ...
  it('rejects when unauthorized (non-ADMIN)', async () => {
    ...
    const res = await client(shwToken).post('/api/v1/hospitals').send(validHospitalPayload);
  ...
  it('rejects when unauthenticated (no token)', async () => {
    const res = await client().post('/api/v1/hospitals').send(validHospitalPayload);
  ...
  it('returns 400 with detailed validation errors', async () => {
    ...
    const res = await client(adminToken).post('/api/v1/hospitals').send(invalidPayload);
